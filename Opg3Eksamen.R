...
# Funktion til at beregne hældningen og interceptet for en linje givet to punkter
...
get_trekanter_for_spiller <- function(afsender) {
  result <- data.frame(x = numeric(), y = numeric(), stringsAsFactors = FALSE)
...
trekanter <- get_trekanter_for_spiller(7)

 # Antag at OB_spillere og Viborg_spillere dataframes har kolonnerne 'PlayerX' og 'PlayerY' for spillerpositioner
plot_shadow_trekanter <- function(OB_spillere, Viborg_spillere, trekanter) {
  # Start med at tegne en fodboldbane
  p <- ggplot() +
    annotate_pitch(colour = "white", fill = "forestgreen") +
    theme_minimal() +
    labs(title = "Fodboldbane med Skyggetrekanter")
  
  # Tilføj OB-spillere som blå punkter
  if (!is.null(OB_spillere) && "PlayerX" %in% names(OB_spillere) && "PlayerY" %in% names(OB_spillere)) {
    p <- p + geom_point(data = OB_spillere, aes(x = PlayerX, y = PlayerY), color = "blue", size = 3)
  }
  
  # Tilføj Viborg-spillere som røde punkter
  if (!is.null(Viborg_spillere) && "PlayerX" %in% names(Viborg_spillere) && "PlayerY" %in% names(Viborg_spillere)) {
    p <- p + geom_point(data = Viborg_spillere, aes(x = PlayerX, y = PlayerY), color = "red", size = 3)
  }
  
  # Tilføj trekanter, hvis de findes
  if (!is.null(trekanter) && nrow(trekanter) > 0) {
    p <- p + geom_polygon(data = trekanter, aes(x = x, y = y), fill = "grey", alpha = 0.5)
    #p <- p + geom_polygon(data = trekanter, aes(x = x, y = y, group = interaction(trekanter$group_id)), fill = "grey", alpha = 0.5)
  }
...
# Eksekver funktionen med dine data
plot_shadow_trekanter(OB_spillere, Viborg_spillere, trekanter)
